...
APP_KEY = 'eLQyR0IeGlCvg1YrDOQ9BRHd8' # API Key
APP_SECRET = 'KSMMz6kfh3amhkABOvVgwwVHWomN9Yi0fnpHTYnljik3Si4Eot' # API Secret Key
OAUTH_TOKEN = '326310066-Gc5Smm4Dg7RIPDaUtch5VcZ2ln16xElVTrcl3ssI' # Access Token
OAUTH_TOKEN_SECRET = 'Fs7m27MNMsTVouG6bgQmJxzKAW0kG61BdejbCrzqm682Y' # Access Token Secret
...
dbStringConnection = "mongodb+srv://davidviu:cristoa95@cluster0.scpww.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
...
for s in twitter_accounts[:len(twitter_accounts)]:
    ...
    while page < 17:
        ...
        for entry in d:
            ...
            t = json.dumps(entry)
            now= str(datetime.now()) #obtener la fecha y ahora actualizado
            dataenviar={
                    'Timestamp': now,
                    'U_Estudiante' : 'Ronald Jimenez'
            }
            #Crear JSON de tweet para MongoDB
            json_sub=json.loads(t) # cargar los datos en JSON
            json_sub.update(dataenviar)#actualizar los datos 
            json_sub2= json.dumps(json_sub)#Convrtir para Mongo

            loaded_entry=json.loads(json_sub2)
            ...
            try:
                 ...
                 gmt=time.gmtime()
                 print("David BD")  #Usuario realizado la BD
                 print ("gmt:",gmt) #La timestamp por cada tweet a la BD
            except pymongo.errors.DuplicateKeyError as e:
                print (e, '\n')
                duplicates += 1
                if duplicates>9:
